...
def multi_task_tester(test_loader, multi_task_model, device, opts):
    ...
    Fusion_save_dir = os.path.join(save_root)
    ...
    with torch.no_grad():  # operations inside don't track history
        for it, (img_ir, img_vi, label, img_names) in enumerate(test_bar):
            ...
            Seg_pred, out1, out2, out3, out4, out5, out6, fused_img = multi_task_model(img_vi, img_ir)  # -----wly
            ...
            # fused_img = YCbCr2RGB(fused_img, vi_Cb, vi_Cr) # ----wly
            # if not is_rgb:
            ...
            for i in range(len(img_names)):
                ...
                # save_img_single(fused_img[i, ::], fusion_save_name)  # ----wly
        ...
        print('IoU:', IoU_list, 'mIoU:', mIoU, 'ACC: {:.4f}'.format(Acc))
