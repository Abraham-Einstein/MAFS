...
def resume(model, optimizer=None, model_save_path=None, device=None, is_train=True):
    ...
    # model.load_state_dict(checkpoint['MTAN'])  # -----wly
    model.load_state_dict(checkpoint['MTAN'], strict=False)
    if is_train:
...
class Saver():
  ...
  # save model
  def write_model(self, ep, total_it, model, best_mIoU, device, is_best=True, logger=None):
    model = model.cpu()
