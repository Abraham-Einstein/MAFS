...
def Fusion_loss(ir, vi, fu, mask, weights=[1, 10, 10], device=None):
    ...
    loss_total = weights[1] * loss_grad + weights[2] * loss_intensity
...
def Re_loss(img1, img2, weights=[5, 5], mask=None, ir_flag=False):
    ...
    # 计算掩膜区域的损失
    if mask is not None:
        loss_intensity = F.l1_loss(img1 * mask, img2 * mask)
        loss_grad = F.l1_loss(grad1 * mask, grad2 * mask)
    else:
...
# ----------------- cwd loss between T and S logits map ------------------------- #
...
class eeemodelLoss(nn.Module):
    def __init__(self):
        ...
        # self.class_weight_semantic_pst900 = torch.from_numpy(
        #     np.array([1.4543, 43.8739, 34.2410, 47.3663, 27.4869])).float()

        self.sdm_loss = L1Loss().cuda()

    def forward(self, inputs, targets):
        binary_gt = targets
        cls = inputs
        # print(binary_gt.shape, cls.shape)
        with torch.no_grad():
            # binary = binary_gt.unsqueeze(1)
            binary = binary_gt
            gt_dis = compute_sdf(binary.cpu().numpy(), binary.shape)
            gt_dis1 = torch.from_numpy(gt_dis).float().cuda()
        # print(gt_dis1.shape)
        loss4 = self.sdm_loss(cls, gt_dis1)
        loss = loss4
        # print(loss)
        return loss
